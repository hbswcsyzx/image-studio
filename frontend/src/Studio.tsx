...
export default function Studio(props: Props) {
  ...
  function addReferences(event: ChangeEvent<HTMLInputElement>) {
    ...
    setReferences(current => [...current, ...files].slice(0, Math.max(0, maxReferenceImages - referencedAssets.length)))
    event.target.value = ''
  }

  function citeAsset(asset: Asset) {
    if (referencedAssets.some(item => item.id === asset.id)) return
    if (references.length + referencedAssets.length >= maxReferenceImages) { setError(`参考图总数不能超过 ${maxReferenceImages} 张`); return }
